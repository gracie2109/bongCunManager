<template>
    ...
    <SearchWrap>
        <SearchView placeholder="Search by store" @reset="onReset" @clear-filter="clearFilter" @setOpen="setOpen"
            :addNew="{
                type: 'function',
                content: null,
            }" :table="table" />
    ...
    <ServiceForm @change-open="() => (open = !open)" :open="open && !selectedItem" />
    <DialogConfirm @change-open="() => {
            selectedItem = null;
            open = !open;
        }
        " :open="open && selectedItem" :title="selectedItem?.name" @cancel="() => {
            selectedItem = null;
            open = false;
        }
        " @delete="handleDelete" />
...
const columns: ColumnDef<any>[]  = reactive([
    ...
    },
    {
        id: 'function',
        header: '',
        cell: ({row}) => h(ActionRow, {row, type: type}),
